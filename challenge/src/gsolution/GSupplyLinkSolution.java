...
public class GSupplyLinkSolution {
	...
	protected GSupplyLinkProblem supplyLinkProblem;

	/*
	 * Structure of a feasible schedule : first job is indice 0
	 */
	protected GJobVariable[] processingSchedule;
	

	/*
	 * Number of batches
	 */
	protected int nbrBatch;
	 ...
	 */
	protected boolean debugConstraints=true; 
	...
	public GSupplyLinkSolution(GSupplyLinkProblem supplyLinkProblem,
			...
			double evaluation, double evaluationTransporter,
			double evaluationCustomer, boolean debugConstraints) {
	...
	public GSupplyLinkSolution(GSupplyLinkSolution sol) {
		this.supplyLinkProblem = sol.supplyLinkProblem;
		this.processingSchedule = new GJobVariable[sol.processingSchedule.length] ;
		for (int i=0;i<sol.processingSchedule.length;i++) 
				this.processingSchedule[i] = new GJobVariable(sol.processingSchedule[i]) ;
		this.nbrBatch = sol.nbrBatch;
		this.evaluation = sol.evaluation;
		this.evaluationTransporter = sol.evaluationTransporter;
		this.evaluationCustomer = sol.evaluationCustomer;
		this.debugConstraints = sol.debugConstraints ;
	}
	...
	public String toString() {
		...
		for (int i=0;i<supplyLinkProblem.getN();i++) {
			st = st + processingSchedule[i].getBatchIndice() ;
	...
	public GSupplyLinkSolution clone() {
		return new GSupplyLinkSolution(this);
	}
	...
	protected BatchInformation[] tabBatchInformation ;
	...
	private boolean checkConstraints() {
		...
		for (int i=0;i<supplyLinkProblem.getN();++i) {
			...
			if (batchTabIndice<0 || batchTabIndice>=getNbrBatch()) {
			...
			tabBatchInformation[batchTabIndice].size += supplyLinkProblem.getJobData()[i].getSize() ;
			
			// Constraint (2) : Update the completion time of the batch if the due date of the job is less 
			if (tabBatchInformation[batchTabIndice].completionTime > supplyLinkProblem.getJobData()[i].getDueDate()) {
				tabBatchInformation[batchTabIndice].completionTime = supplyLinkProblem.getJobData()[i].getDueDate() ;
			}
		}
		...
		for (int b=getNbrBatch()-1;b>=0;--b) {
			...
			if (tabBatchInformation[b].size>supplyLinkProblem.getTransporter(0).getCapacity()){
				if (debugConstraints) {					
					System.out.println("batch "+b+" : tabBatchInformation[b].size="+tabBatchInformation[b].size) ;
					System.out.println("constraints not satisfied: transporter capacity exceeded") ;
				}
				return false ;
			}
